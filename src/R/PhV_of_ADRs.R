...
#' Compute PhV statistics for adverse reactions to all drugs in ATC encoded drug class taking 
#' 
...
#' 
#' source ~/Scripts/bash/submit_Rjob_3arg.sh PhV_of_ADRs.R "ANTIEPILEPTICS" 3 "dev_age_cat"
#' 
#' 
...
cat(paste0(format(nrow(unique(aeolus[drug_concept_id %in% drugs_to_limit_by,
                                .(.N),
                                by=.(drug_concept_id,outcome_concept_id,id)][
                                  ,.(drug_concept_id,outcome_concept_id,id)
                                  ])),big.mark=",")," reports of ",format(nrow(adr_pairs),big.mark=",")," AEDARs"))
...
filename <- paste0("PhV_of_ADRs_stratified_by_",group,".csv")
